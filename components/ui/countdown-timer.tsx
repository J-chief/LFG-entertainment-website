...
export default function CountdownTimer({ targetDate }: { targetDate: string }) {
  ...
  return (
    <div className="flex items-center justify-center gap-4 md:gap-8 font-display">
      {/* Days */}
      <CountdownSegment value={timeLeft.days} label="DAYS" />
      <span className="text-3xl md:text-5xl font-black text-white/40 -translate-y-2">:</span>
      {/* Hours */}
...
function CountdownSegment({ value, label }: { value: string; label: string }) {
  return (
    <div className="flex flex-col items-center">
      {/* Digits row */}
      <div className="flex overflow-hidden h-14 md:h-20 items-center justify-center">
...
function Digit({ char }: { char: string }) {
  return (
    <div className="relative w-8 md:w-12 h-14 md:h-20 flex items-center justify-center">
